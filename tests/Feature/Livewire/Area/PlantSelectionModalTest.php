...
describe('PlantSelectionModal', function (): void {
    ...
    describe('Modal Management', function (): void {
        ...
        it('can close modal', function (): void {
            Livewire::test(PlantSelectionModal::class, ['area' => $this->area])
                ->set('showModal', true)
                ->set('search', 'test')
                ->set('selectedPlants', [1 => ['quantity' => 2, 'notes' => 'test']])
                ->call('closeModal')
                ->assertSet('showModal', false)
                ->assertSet('search', '')
                ->assertSet('selectedPlantTypeId', null)
        ...
        it('resets state when opening modal', function (): void {
            Livewire::test(PlantSelectionModal::class, ['area' => $this->area])
                ->set('search', 'existing search')
                ->set('selectedPlantTypeId', 999)
                ->set('selectedPlants', [1 => ['quantity' => 5, 'notes' => 'old notes']])
                ->call('openModal')
                ->assertSet('showModal', true)
                ->assertSet('search', '')
                ->assertSet('selectedPlantTypeId', null)
    ...
    describe('Plant Data Management', function (): void {
        beforeEach(function (): void {
            $this->component = Livewire::test(PlantSelectionModal::class, ['area' => $this->area])
                ->call('togglePlant', $this->plant1->id);
        });
    ...
    describe('Adding Plants to Area', function (): void {
        ...
        it('shows success message after adding plants', function (): void {
            ...
            // Check that plants were actually added and modal was closed
